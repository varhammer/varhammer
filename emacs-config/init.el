;; init.el — Varhammer 1.1.8 "Midnight" portable development environment:
...
(let* ((init-dir (file-name-directory (or load-file-name
  ...
  (defun varhammer-toggle-theme ()
    "Toggle between varhammer and spolsky themes."
    (interactive)
    (if (memq 'varhammer custom-enabled-themes)
        (progn
          (disable-theme 'varhammer)
          (load-theme 'spolsky t)
          (message "Theme switched to: spolsky"))
      (progn
        (disable-theme 'spolsky)
        (load-theme 'varhammer t)
        (message "Theme switched to: varhammer"))))
